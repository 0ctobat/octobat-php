...
class CurlClient implements ClientInterface
{
    ...
    public function request($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (($method == 'post') && (Octobat::$maxNetworkRetries > 0)) {
        ...
        $opts[CURLOPT_CAINFO] = Octobat::getCABundlePath();
        if (!Octobat::getVerifySslCerts()) {
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Octobat ($url).  Please check your "
                 . "internet connection and try again.  If this problem persists, "
                 . "you should check Octobat's service status at "
                 . "https://twitter.com/octobatapp, or";
                break;
            case CURLE_SSL_CACERT:
            case CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Octobat's SSL certificate.  Please make sure "
                 . "that your network is not intercepting certificates.  "
                 . "(Try going to $url in your browser.)  "
                 . "If this problem persists,";
                break;
            default:
                $msg = "Unexpected error communicating with Octobat.  "
                 . "If this problem persists,";
        }
        $msg .= " let us know at tech@octobat.com.";
    ...
    {
        if ($numRetries >= Octobat::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime($numRetries)
    {
        ...
        $sleepSeconds = min(
            Octobat::getInitialNetworkRetryDelay() * 1.0 * pow(2, $numRetries - 1),
            Octobat::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = max(Octobat::getInitialNetworkRetryDelay(), $sleepSeconds);
