 ...
 * @package Octobat
 */
class OctobatObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    protected $_opts;
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToOctobatObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_values) && array_key_exists($k, $this->_values)) {
            ...
            $message = "Octobat Notice: Undefined property of $class instance: $k. "
                    . "HINT: The $k attribute was set in the past, however. "
                    . "It was then wiped when refreshing the object "
                    . "with the result returned by Octobat's API, "
                    . "probably as a result of a save(). The attributes currently "
                    . "available on this object are: $attrs";
            Octobat::getLogger()->error($message);
            return $nullval;
        } else {
            $class = get_class($this);
            Octobat::getLogger()->error("Octobat Notice: Undefined property of $class instance: $k");
            return $nullval;
        }
    }
     ...
     *
     * @return OctobatObject The object constructed from the given values.
     */
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof OctobatObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a OctobatObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if ($k === "metadata") {
                $this->_values[$k] = OctobatObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToOctobatObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a OctobatObject. A OctobatObject may contain modified
            //      values within in that its parent OctobatObject doesn't know about.
            //
            $original = array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof OctobatObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if ($value === null) {
            ...
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToOctobatObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof OctobatObject) {
    ...
    {
        if ($recursive) {
            return Util\Util::convertOctobatObjectToArray($this->_values);
        } else {
            return $this->_values;
        }
    }
    ...
    /**
     * Sets all keys within the OctobatObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any OctobatObjects contained as values or
    ...
    protected static function deepCopy($obj)
    {
        if (is_array($obj)) {
            ...
            return $copy;
        } elseif ($obj instanceof OctobatObject) {
    ...
    /**
     * @return object The last response from the Octobat API
     */
     ...
     * @param ApiResponse
     *
     * @return void Set the last response from the Octobat API
     */
